...
window = Tk()
window.title("Hangman Game Made by:-Ritesh and Shubham")
window.resizable(0, 0)

word_list = ['WORLD','COUNTRY','PERFECT','POSITIVITY','NATURAL','MOVIE',
             'CODE','PYTHON',"RABBIT","KITKAT"]
...
txtWord=StringVar()
Entry(window, textvariable=txtWord, font=("Consolas 22 bold")).grid(row=1,column=4, columnspan=4, padx=10)
window.bind('<Return>', guess)


Button(window, text="New\nGame", command=lambda:play(), bg="red",font=("Helvetica 8 bold")).grid(row=3, column=4, sticky="NSWE")
